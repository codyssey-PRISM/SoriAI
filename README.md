...
- 웹(보호자), iOS(어르신) 클라이언트가 같은 백앤드 API 사용
- AI 통화 에이전트는 **VAPI.ai**를 사용
  - iOS에서 VAPI SDK를 사용해서 통화를 연결.
  - 백앤드에서는 **WebHook**으로 Vapi 서버에서 통화 내용을 실시간으로 받음.
- 백앤드에서 **APNS 서버**로 푸시 전송 요청
  - 초대코드 입력 성공 후, 백앤드 전송된 iOS 디바이스ID를 통해 푸시 전송.
- iOS에서는 **PushKit**을 통해서 통화 알림을 받고 **CallKit**을 통해서 실제 전화와 같은 UI/UX를 제공합.
- Vapi.ai는 대화 외부 LLM 모댈과 외부 음성 모델을 선택할 수 있음.
  - 통화과 끝나면 정해진 프로프트로 요약, 태그 추출, 분석 등이 가능함.
